...
class CouponController extends Controller {
  async addNewCoupon(req, res) {
    ...
    if (!coupon?._id)
      throw createHttpError.InternalServerError("Discount code not registered");
  ...
  async updateCoupon(req, res) {
    ...
    if (updateResult.modifiedCount === 0)
      throw createHttpError.InternalServerError("Discount code not updated");
  ...
  async getAllCoupons(req, res) {
    ...
    if (!coupons)
      throw createHttpError.InternalServerError("Discount code not found");
    return res.status(HttpStatus.OK).json({
      statusCode: HttpStatus.OK,
      data: {
        coupons,
  ...
  async findCouponById(id) {
    ...
    if (!coupon)
      throw createHttpError.BadRequest(
        "No discount code found with these specifications"
      );
