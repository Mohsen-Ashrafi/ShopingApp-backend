...
class ProductController extends Controller {
  ...
  async getOneProductBySlug(req, res) {
    ...
    if (!product)
      throw createHttpError.NotFound(
        "No course with these specifications found"
      );
  ...
  async changeProductDiscountStatus(req, res) {
    ...
    throw createHttpError.BadRequest("Change failed, try again");
  }
  ...
  async removeProduct(req, res) {
    ...
    if (deleteResult.deletedCount == 0)
      throw createHttpError.InternalServerError("Product deletion failed");
    return res.status(HttpStatus.OK).json({
      statusCode: HttpStatus.OK,
      data: {
  ...
  async updateProduct(req, res) {
    ...
    if (!updateProductResult.modifiedCount)
      throw createHttpError.InternalServerError("Product update failed");
  ...
  async likeProduct(req, res) {
    ...
    if (productUpdate.modifiedCount === 0 || userUpdate.modifiedCount === 0)
      throw createHttpError.BadRequest("The operation was unsuccessful.");
  ...
  async findProductById(id) {
    if (!mongoose.isValidObjectId(id))
      throw createHttpError.BadRequest("The submitted product ID is not valid");
